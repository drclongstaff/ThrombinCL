...
shinyServer(function(input, output) {      # Set up the Shiny Server
  ...
  CalibF<-reactive({
    ...
    #read.csv("./Data/FDAall.csv")[c(1, 2:13)]
    read.csv("./Data/Aug 3 JH Raw F data.csv")[c(1, 30:31)]
  ...
  RawF0 <- reactive({
    ...
    #read.csv("./Data/FDAall.csv")[c(1, input$datstart:input$datend)]
    read.csv("./Data/Aug 3 JH Raw F data.csv")[c(1, input$datstart:input$datend)]
    else(read.csv(inputFile$datapath)[c(1, input$datstart:input$datend)])
  })
 ...
 # })
  ...
  output$myCalib<-renderPlot({
    ...
    plot(CalibDat()[,1],CalibDat()[,2], pch=2, xlim = c(0, TatD), xlab = "Time", ylab = "Fluorescence",
         main =  paste("Calibrator : initial slope =", round(SlopeInt()[2], 4), " and intercept = ", round(SlopeInt()[1], 4)))
...
###DO THE ANALYSIS ON THE CURVES
 ...
 #write.table(TabResShort , "clipboard",  sep="\t", row.names = FALSE,  col.names=TRUE)
  ...
  output$resultsTable<-renderTable({
    #write.table(TabRes()[,c(1,3:7)] , "clipboard",  sep="\t", row.names = FALSE,  col.names=TRUE)
    TabRes()[,c(1:8, 13)]
    #head(readData())
  })
...
output$rawData<-renderTable({
  ...
  #write.table( rawData[,input$colmnames], "clipboard",  sep="\t", row.names = FALSE,  col.names=FALSE)#select a columnn needs ui changing
  #write.table( rawData, "clipboard",  sep="\t", row.names = FALSE,  col.names=FALSE)#use all columns
  #write.table( rawData[-1], "clipboard",  sep="\t", row.names = FALSE,  col.names=FALSE)#miss out the first column
  rawData
})
